...
Para propósitos de demonstração, testarei `compare` em cenários artificiais: comparando `X` consigo mesma, `X` com `exams` e `X` com uma fatia de si mesma. Ela retorna `TRUE` caso as duas tabelas sejam semelhantes o suficiente e `FALSE` caso contrário, fora uma mensagem explicando o resultado (qual parâmetro foi considerado diferente demais em caso de falha; o nome do parâmetro segue o mesmo formato que as colunas das tabelas retornadas por `examine`).
